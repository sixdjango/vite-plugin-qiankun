...
const createQiankunHelper = (qiankunName: string) => `
  const createDeffer = (hookName) => {
    const d = new Promise((resolve, reject) => {
      window && (window[\`vite\${hookName}\`] = resolve)
    })
...
const createImportFinallyResolve = (qiankunName: string) => {
  return `
    ...
    if (qiankunLifeCycle) {
      window.vitemount((props) => qiankunLifeCycle.mount(props));
      window.viteunmount((props) => qiankunLifeCycle.unmount(props));
      window.vitebootstrap(() => qiankunLifeCycle.bootstrap());
    }
  `;
};
...
const htmlPlugin: PluginFn = (qiankunName, microOption) => {
  ...
  const module2DynamicImport = ($: CheerioAPI, scriptTag: Element) => {
    ...
    if (microOption.useDevMode && !isProduction) {
      appendBase = "(window.__INJECTED_PUBLIC_PATH_BY_QIANKUN__ ? (window.__INJECTED_PUBLIC_PATH_BY_QIANKUN__ + '..') : '') + ";
    }
